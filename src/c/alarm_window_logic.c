...
static void take_dose(ClickRecognizerRef recognizer, void* context)
{
    APP_LOG(APP_LOG_LEVEL_DEBUG, "take dose requested");
    vibes_cancel();
    m_alarm_silenced = true;
    schedule_next_alarm();
    save_data();
...
void alarm_window_click_config_provider(void* context)
{
    ...
    window_single_click_subscribe(BUTTON_ID_SELECT, take_dose);
...
char* get_wakeup_alarm_time_string()
{
    ...
    fill_time_string(alarm_time_buffer, t->tm_hour, t->tm_min);
